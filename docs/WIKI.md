...
6. [Шаги (Steps)](#шаги-steps)
7. [Условия (Conditions)](#условия-conditions)
8. [Циклы (ForEach)](#циклы-foreach)
9. [Вызов задач (Call)](#вызов-задач-call)
10. [Группы (Group)](#группы-group)
11. [Параллельное выполнение (Parallel)](#параллельное-выполнение-parallel)
12. [Управление секретами (Secrets)](#управление-секретами-secrets)
13. [Хуки жизненного цикла (Hooks)](#хуки-жизненного-цикла-hooks)
...
- **Импортов** — переиспользование переменных и задач из других файлов
- Переменных и выражений
- Условного выполнения (if/else)
- Циклов (foreach)
- Параллельного выполнения (parallel)
...
Система импортов позволяет разбить workflow на несколько файлов и переиспользовать переменные и задачи между проектами.
...
imports:
  - ./common_vars.yaml           # Относительный путь
  - ./helpers.yaml               # Файл с задачами
  - ../shared/config.yaml        # Родительская директория
...
При загрузке workflow с импортами:

1. Загружаются все импортируемые файлы (рекурсивно)
2. Переменные из импортов объединяются с переменными основного файла
3. Задачи из импортов становятся доступны в основном файле
4. При конфликте имён переменных — приоритет у основного файла
5. При конфликте имён задач — ошибка (дублирование не допускается)

### Пример: Файл с общими переменными
...
### Пример: Файл с вспомогательными задачами
...
### Пример: Основной файл с импортами
...
variables:
  # Переопределяем переменную из импорта
  api_base: https://production.api.example.com
  ...
  main:
    steps:
      - step:
          ...
          uses: log.info
          with:
            message: "🚀 Starting ${app_name} in ${environment}"

      # Используем задачу из helpers.yaml
      - call:
          id: report_success
          task: log_success
          with:
            message: "All checks passed for ${app_name}"
```

### Вложенные импорты

Импорты могут быть вложенными. Если `main.yaml` импортирует `helpers.yaml`, который в свою очередь импортирует `common_vars.yaml`, то переменные и задачи из `common_vars.yaml` будут доступны в `main.yaml`.
  ...
  make_api_call:
    steps:
      - step:
          ...
          uses: http.request
          with:
            url: "${api_base}/data"    # api_base из common_vars.yaml
            timeout: "${timeout}"       # timeout из common_vars.yaml
```

### Циклические импорты

Циклические импорты автоматически обнаруживаются и вызывают ошибку:

```
Ошибка: Обнаружен циклический импорт: /path/to/file.yaml
```

### Относительные пути

Поддерживаются различные форматы путей:

```yaml
imports:
  - ./file.yaml              # Текущая директория
  - ../parent/file.yaml      # Родительская директория
  - subdir/file.yaml         # Поддиректория
  - /absolute/path.yaml      # Абсолютный путь (не рекомендуется)
```

### Лучшие практики

1. **Один файл — одна ответственность**
   - `common_vars.yaml` — только переменные
   - `helpers.yaml` — вспомогательные задачи
   - `api_tasks.yaml` — задачи для работы с API

2. **Избегайте глубоких вложенностей**
   - Максимум 2-3 уровня импортов

3. **Документируйте импорты**
   ```yaml
   imports:
     - ./common_vars.yaml    # Общие переменные и константы
     - ./api_helpers.yaml    # Задачи для работы с API
     - ./validators.yaml     # Задачи для валидации
   ```

4. **Используйте осмысленные имена**
   - Задачи из импортов должны иметь уникальные имена
   - Добавляйте префикс при необходимости: `api_get_user`, `db_connect`
  ...
  helper_task:
    inputs:
      ...
      api_token:
        type: string
        required: true
        secret: true              # Помечает параметр как секрет
...
#### Поля входных параметров

| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `type` | string | Да | Тип данных: `string`, `integer`, `boolean`, `array`, `object` |
| `required` | bool | Нет | Обязателен ли параметр (по умолчанию: `false`) |
| `default` | any | Нет | Значение по умолчанию |
| `secret` | bool | Нет | Является ли секретом (будет маскироваться) |
...
Параллельное выполнение позволяет запускать несколько шагов одновременно для ускорения работы.
...
- parallel:
    id: parallel_tasks
    max_concurrency: 3           # Максимальное количество параллельных задач
    error_mode: fail_fast        # Режим обработки ошибок: fail_fast | continue
    steps:
      - step:
          id: task_1
          uses: http.request
...
| `max_concurrency` | int | Нет | Максимальное количество одновременно выполняемых шагов (по умолчанию: не ограничено) |
| `error_mode` | string | Нет | Режим обработки ошибок: `fail_fast` (прервать все при ошибке) или `continue` (продолжить остальные) |
| `steps` | array | Да | Список шагов для параллельного выполнения |

### Режимы обработки ошибок

#### fail_fast (по умолчанию)

При ошибке в любом шаге:
1. Прерываются все выполняющиеся шаги
2. Не запускаются оставшиеся шаги
3. Parallel-блок завершается с ошибкой
...
При ошибке в любом шаге:
1. Остальные шаги продолжают выполнение
2. Результаты собираются от всех шагов
3. Parallel-блок завершается успешно, если хотя бы один шаг успешен
...
Используйте `max_concurrency` для ограничения нагрузки:

```yaml
- parallel:
    id: fetch_data
    max_concurrency: 5           # Не более 5 одновременных запросов
    steps:
      - step:
          id: fetch_1
          uses: http.request
...
AutoFlow.NET предоставляет встроенную систему управления секретами для безопасной работы с конфиденциальными данными.
...
AutoFlow.NET поддерживает несколько источников секретов:

#### 1. Переменные окружения (EnvSecretProvider)

Секреты читаются из переменных окружения:
...
#### 3. Комбинированный провайдер (CompositeSecretProvider)

Последовательно опрашивает несколько провайдеров:
...
```
[INFO] API call with token: ***MASKED***
[INFO] Connecting to database with password: ***MASKED***
```

### Входные параметры с секретами

Определите входные параметры как секреты для валидации:
...
variables:
  # Секреты
  db_connection: "Server=${secret:DB_HOST};Password=${secret:DB_PASSWORD}"
...
Хуки позволяют выполнять自定义 код на различных этапах выполнения workflow.
...
public interface IWorkflowLifecycleHook
{
    ...
    Task OnWorkflowStartingAsync(WorkflowContext context);
    Task OnWorkflowCompletedAsync(WorkflowContext context);
    Task OnWorkflowFailedAsync(WorkflowContext context, Exception error);
    
    // Step-level hooks
    Task OnStepStartingAsync(StepContext context);
    Task OnStepCompletedAsync(StepContext context);
    Task OnStepFailedAsync(StepContext context, Exception error);
}
```
...
Логирует все события workflow:

```csharp
services.AddSingleton<IWorkflowLifecycleHook, LoggingHook>();
```

Вывод:
```
[Workflow] Starting: deployment_pipeline
[Step] Starting: load_config (files.read)
[Step] Completed: load_config in 150ms
[Workflow] Completed: deployment_pipeline in 2.5s
```

#### MetricsHook

Собирает метрики выполнения:

```csharp
services.AddSingleton<IWorkflowLifecycleHook, MetricsHook>();
```

Собирает:
- Время выполнения workflow
- Время выполнения каждого шага
- Количество успешных/неуспешных шагов
- Среднее время выполнения по типам keywords

### Создание кастомного хука

```csharp
public class NotificationHook : IWorkflowLifecycleHook
{
    public string Name => "Notification";
    
    public async Task OnWorkflowCompletedAsync(WorkflowContext context)
    {
        // Отправить уведомление об успешном завершении
        await SendSlackNotification($"Workflow {context.WorkflowName} completed!");
    }
    
    public async Task OnWorkflowFailedAsync(WorkflowContext context, Exception error)
    {
        // Отправить уведомление об ошибке
        await SendSlackNotification($"Workflow {context.WorkflowName} failed: {error.Message}");
    }
    
    // Остальные методы с пустой реализацией...
}
```
...
services.AddSingleton<IWorkflowLifecycleHook, LoggingHook>();
services.AddSingleton<IWorkflowLifecycleHook, MetricsHook>();
services.AddSingleton<IWorkflowLifecycleHook, NotificationHook>();
...
### Retry с расширенными возможностями
...
- step:
    ...
    retry:
      attempts: 5
      delay: "10s"
      type: exponential          # Тип задержки: fixed | exponential
      backoff_multiplier: 2.0    # Множитель для exponential (по умолчанию: 2.0)
      max_delay: "60s"           # Максимальная задержка
      retry_on:                  # Повторять только при этих ошибках
        - "timeout"
        - "503"
      skip_on:                   # Пропустить retry при этих ошибках
        - "401"
        - "403"
```
...
```
Попытка 1: 10s
Попытка 2: 20s (10s × 2.0)
Попытка 3: 40s (20s × 2.0)
Попытка 4: 60s (40s × 2.0, но max_delay = 60s)
```

### Обработчики ошибок (OnError)

Определяют действия при ошибке на уровне задачи:

```yaml
tasks:
  main:
    on_error:                     # Выполняется при любой ошибке в задаче
      - step:
          id: notify_error
          uses: http.request
          with:
...
- step:
    ...
    with:
      url: "${api_url}"
    on_error:                     # Выполняется только при ошибке этого шага
      - step:
          id: use_fallback
          uses: http.request
          with:
